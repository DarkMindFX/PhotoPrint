...
namespace PPT.Functions.Address.V1
{
    public class Erase : FunctionBase
    {
        ...
        public Erase(IHttpContextAccessor httpContextAccessor, IAddressDal dalAddress) : base(httpContextAccessor)
        {
            _dalAddress = dalAddress;
        }

        [Authorize]
        [FunctionName("AddressesErase")]
            ...
            ILogger log)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    bool isRemoved = _dalAddress.Erase(id);
