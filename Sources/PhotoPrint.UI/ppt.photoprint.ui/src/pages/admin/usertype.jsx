...
class UserTypePage extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = { 
            ...
            usertype: this._createEmptyUserTypeObj(),
            ...
            error: null,
            success: null,
            urlEntities: `${rooPath}/usertypes`,
            urlThis: `${rooPath}/usertype/${paramOperation}` + (paramId ? `/${paramId}` : ``)
        };

        this.onUserTypeNameChanged = this.onUserTypeNameChanged.bind(this);
        this.onIsDeletedChanged = this.onIsDeletedChanged.bind(this);
        this._getUserType = this._getUserType.bind(this);
    ...
    onUserTypeNameChanged(event) {

        let updatedState = this.state;
        let newVal = null;
        newVal = event.target.value
        updatedState.usertype.UserTypeName = newVal;
    ...
    onIsDeletedChanged(event) {
        ...
        let newVal = null;
        newVal = event.target.value
        updatedState.usertype.IsDeleted = newVal;
    ...
    onSaveClicked() {
        ...
        console.log("Saving UserType: ", this.state.usertype);
        
        if(this._validateForm()) {
            const reqUserType = new UserTypeDto();
            reqUserType.ID = this.state.id;
            reqUserType.UserTypeName = this.state.usertype.UserTypeName;
            reqUserType.IsDeleted = this.state.usertype.IsDeleted;

            console.log("Saving UserType: ", reqUserType); 
        
            let dalUserTypes = new UserTypesDal();

            let obj = this;

            function upsertUserTypeThen(response) {
                ...
                if(response.status == constants.HTTP_OK || response.status == constants.HTTP_Created) {
                    ...
                    if(response.status == constants.HTTP_Created) {
                        ...
                        updatedState.success = `UserType was created. ID: ${updatedState.id}`;
                    }
                    else {
                        updatedState.success = `UserType was updated`;                
                    }
            ...
            if(this.state.id != null) {
                dalUserTypes.updateUserType(reqUserType)
                                        .then( (res) => { upsertUserTypeThen(res); } )
                                        .catch( (err) => { upsertCatch(err); });
            }
            else {
                dalUserTypes.insertUserType(reqUserType)
                                        .then( (res) => { upsertUserTypeThen(res); } )
                                        .catch( (err) => { upsertCatch(err); });        
            }
    ...
    onDeleteConfirm() {  
        ...
        dalUserTypes.deleteUserType(this.state.id).then( (response) => {
            ...
            <div>
                 <table>
                    <tbody>
                        ...
                        <tr>
                            <td colSpan={2}>
                                <TextField  id="UserTypeName" 
                                            fullWidth
                                            type="text" 
                                            variant="filled" 
                                            label="UserTypeName" 
                                            value={this.state.usertype.UserTypeName}
                                            onChange={ (event) => { this.onUserTypeNameChanged(event) } }
                                            />
                        ...
                        <tr>
                            <td colSpan={2}>
                                <TextField  id="IsDeleted" 
                                            ...
                                            variant="filled" 
                                            label="IsDeleted" 
                                            value={this.state.usertype.IsDeleted}
                ...
                <Dialog open={this.state.showDeleteConfirm} onClose={() => { this.onDeleteCancel() }} aria-labelledby="form-dialog-title">
                    <DialogTitle id="form-dialog-title">Delete UserType</DialogTitle>
                    <DialogContent>
                    <DialogContentText>
    ...
    _createEmptyUserTypeObj() {
        let usertype = new UserTypeDto();
    ...
    async _getUserType()
    {
        if(this.state.id) {
            ...
            let dalUserTypes = new UserTypesDal();
            let response = await dalUserTypes.getUserType(this.state.id);

            if(response.status == constants.HTTP_OK)
            {
                updatedState.usertype = response.data;                
            }
