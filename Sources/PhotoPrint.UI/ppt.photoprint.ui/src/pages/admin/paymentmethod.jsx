...
class PaymentMethodPage extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = { 
            ...
            paymentmethod: this._createEmptyPaymentMethodObj(),
            ...
            error: null,
            success: null,
            urlEntities: `${rooPath}paymentmethods`,
            urlThis: `${rooPath}paymentmethod/${paramOperation}` + (paramId ? `/${paramId}` : ``)
        };
    ...
    onNameChanged(event) {
        ...
        updatedState.paymentmethod.Name = newVal;
    ...
    onDescriptionChanged(event) {
        ...
        let newVal = null;
        newVal = event.target.value
        updatedState.paymentmethod.Description = newVal;
    ...
    onSaveClicked() {
        ...
        console.log("Saving PaymentMethod: ", this.state.paymentmethod);
        
        if(this._validateForm()) {
            const reqPaymentMethod = new PaymentMethodDto();
            reqPaymentMethod.ID = this.state.id;
            reqPaymentMethod.Name = this.state.paymentmethod.Name;
            reqPaymentMethod.Description = this.state.paymentmethod.Description;
            reqPaymentMethod.IsDeleted = this.state.paymentmethod.IsDeleted;

            console.log("Saving PaymentMethod: ", reqPaymentMethod); 
        
            let dalPaymentMethods = new PaymentMethodsDal();

            let obj = this;

            function upsertPaymentMethodThen(response) {
                ...
                if(response.status == constants.HTTP_OK || response.status == constants.HTTP_Created) {
                    ...
                    if(response.status == constants.HTTP_Created) {
                        ...
                        updatedState.success = `PaymentMethod was created. ID: ${updatedState.id}`;
                    }
                    else {
                        updatedState.success = `PaymentMethod was updated`;                
                    }
            ...
            if(this.state.id != null) {
                dalPaymentMethods.updatePaymentMethod(reqPaymentMethod)
                                        .then( (res) => { upsertPaymentMethodThen(res); } )
                                        .catch( (err) => { upsertCatch(err); });
            }
            else {
                dalPaymentMethods.insertPaymentMethod(reqPaymentMethod)
                                        .then( (res) => { upsertPaymentMethodThen(res); } )
                                        .catch( (err) => { upsertCatch(err); });        
            }
    ...
    onDeleteConfirm() {  
        ...
        dalPaymentMethods.deletePaymentMethod(this.state.id).then( (response) => {
        ...
        return (
            <div>
                ...
                <Dialog open={this.state.showDeleteConfirm} onClose={() => { this.onDeleteCancel() }} aria-labelledby="form-dialog-title">
                    <DialogTitle id="form-dialog-title">Delete PaymentMethod</DialogTitle>
                    <DialogContent>
                    <DialogContentText>
    ...
    _createEmptyPaymentMethodObj() {
        let paymentmethod = new PaymentMethodDto();
    ...
    async _getPaymentMethod()
    {
        if(this.state.id) {
            ...
            let dalPaymentMethods = new PaymentMethodsDal();
            let response = await dalPaymentMethods.getPaymentMethod(this.state.id);

            if(response.status == constants.HTTP_OK)
            {
                updatedState.paymentmethod = response.data;                
            }
...
export default withRouter(PaymentMethodPage);