...
class UserStatusPage extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = { 
            ...
            userstatus: this._createEmptyUserStatusObj(),
            ...
            error: null,
            success: null,
            urlEntities: `${rooPath}userstatuses`,
            urlThis: `${rooPath}userstatus/${paramOperation}` + (paramId ? `/${paramId}` : ``)
        };

        this.onStatusNameChanged = this.onStatusNameChanged.bind(this);
        this.onIsDeletedChanged = this.onIsDeletedChanged.bind(this);
        this._getUserStatus = this._getUserStatus.bind(this);
    ...
    onStatusNameChanged(event) {

        let updatedState = this.state;
        let newVal = null;
        newVal = event.target.value
        updatedState.userstatus.StatusName = newVal;
    ...
    onIsDeletedChanged(event) {
        ...
        let newVal = null;
        newVal = event.target.value
        updatedState.userstatus.IsDeleted = newVal;
    ...
    onSaveClicked() {
        ...
        console.log("Saving UserStatus: ", this.state.userstatus);
        
        if(this._validateForm()) {
            const reqUserStatus = new UserStatusDto();
            reqUserStatus.ID = this.state.id;
            reqUserStatus.StatusName = this.state.userstatus.StatusName;
            reqUserStatus.IsDeleted = this.state.userstatus.IsDeleted;

            console.log("Saving UserStatus: ", reqUserStatus); 
        
            let dalUserStatuses = new UserStatusesDal();

            let obj = this;

            function upsertUserStatusThen(response) {
                ...
                if(response.status == constants.HTTP_OK || response.status == constants.HTTP_Created) {
                    ...
                    if(response.status == constants.HTTP_Created) {
                        ...
                        updatedState.success = `UserStatus was created. ID: ${updatedState.id}`;
                    }
                    else {
                        updatedState.success = `UserStatus was updated`;                
                    }
            ...
            if(this.state.id != null) {
                dalUserStatuses.updateUserStatus(reqUserStatus)
                                        .then( (res) => { upsertUserStatusThen(res); } )
                                        .catch( (err) => { upsertCatch(err); });
            }
            else {
                dalUserStatuses.insertUserStatus(reqUserStatus)
                                        .then( (res) => { upsertUserStatusThen(res); } )
                                        .catch( (err) => { upsertCatch(err); });        
            }
    ...
    onDeleteConfirm() {  
        ...
        dalUserStatuses.deleteUserStatus(this.state.id).then( (response) => {
            ...
            <div>
                 <table>
                    <tbody>
                        ...
                        <tr>
                            <td colSpan={2}>
                                <TextField  id="StatusName" 
                                            fullWidth
                                            type="text" 
                                            variant="filled" 
                                            label="StatusName" 
                                            value={this.state.userstatus.StatusName}
                                            onChange={ (event) => { this.onStatusNameChanged(event) } }
                                            />
                        ...
                        <tr>
                            <td colSpan={2}>
                                <TextField  id="IsDeleted" 
                                            ...
                                            variant="filled" 
                                            label="IsDeleted" 
                                            value={this.state.userstatus.IsDeleted}
                ...
                <Dialog open={this.state.showDeleteConfirm} onClose={() => { this.onDeleteCancel() }} aria-labelledby="form-dialog-title">
                    <DialogTitle id="form-dialog-title">Delete UserStatus</DialogTitle>
                    <DialogContent>
                    <DialogContentText>
                        Are you sure you want to delete this UserStatus?
                    </DialogContentText>                    
                    </DialogContent>
                    <DialogActions>
    ...
    _createEmptyUserStatusObj() {
        let userstatus = new UserStatusDto();
    ...
    async _getUserStatus()
    {
        if(this.state.id) {
            ...
            let dalUserStatuses = new UserStatusesDal();
            let response = await dalUserStatuses.getUserStatus(this.state.id);

            if(response.status == constants.HTTP_OK)
            {
                updatedState.userstatus = response.data;                
            }
