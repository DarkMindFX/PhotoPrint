...
class CategoryPage extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = { 
            ...
            category: this._createEmptyCategoryObj(),
            ...
            error: null,
            success: null,
            urlEntities: `${rooPath}categories`,
            urlThis: `${rooPath}category/${paramOperation}` + (paramId ? `/${paramId}` : ``)
        };
    ...
    onCategoryNameChanged(event) {
        ...
        updatedState.category.CategoryName = newVal;
    ...
    onDescriptionChanged(event) {
        ...
        let newVal = null;
        newVal = event.target.value
        updatedState.category.Description = newVal;
    ...
    onParentIDChanged(event) {
        ...
        updatedState.category.ParentID = newVal;
    ...
    onCreatedDateChanged(event) {
        ...
        let newVal = null;
        newVal = event.target.value
        updatedState.category.CreatedDate = newVal;
    ...
    onCreatedByIDChanged(event) {
        ...
        let newVal = null;
        newVal = parseInt(event.target.value);
        updatedState.category.CreatedByID = newVal;
    ...
    onModifiedDateChanged(event) {
        ...
        let newVal = null;
        newVal = event.target.value
        updatedState.category.ModifiedDate = newVal;
    ...
    onModifiedByIDChanged(event) {
        ...
        let newVal = null;
        newVal = parseInt(event.target.value);
        updatedState.category.ModifiedByID = newVal;
    ...
    onSaveClicked() {
        ...
        console.log("Saving Category: ", this.state.category);
        
        if(this._validateForm()) {
            const reqCategory = new CategoryDto();
            reqCategory.ID = this.state.id;
            reqCategory.CategoryName = this.state.category.CategoryName;
            reqCategory.Description = this.state.category.Description;
            reqCategory.ParentID = this.state.category.ParentID;
            reqCategory.IsDeleted = this.state.category.IsDeleted;
            reqCategory.CreatedDate = this.state.category.CreatedDate;
            reqCategory.CreatedByID = this.state.category.CreatedByID;
            reqCategory.ModifiedDate = this.state.category.ModifiedDate;
            reqCategory.ModifiedByID = this.state.category.ModifiedByID;

            console.log("Saving Category: ", reqCategory); 
        
            let dalCategories = new CategoriesDal();

            let obj = this;

            function upsertCategoryThen(response) {
                ...
                if(response.status == constants.HTTP_OK || response.status == constants.HTTP_Created) {
                    ...
                    if(response.status == constants.HTTP_Created) {
                        ...
                        updatedState.success = `Category was created. ID: ${updatedState.id}`;
                    }
                    else {
                        updatedState.success = `Category was updated`;                
                    }
            ...
            if(this.state.id != null) {
                dalCategories.updateCategory(reqCategory)
                                        .then( (res) => { upsertCategoryThen(res); } )
                                        .catch( (err) => { upsertCatch(err); });
            }
            else {
                dalCategories.insertCategory(reqCategory)
                                        .then( (res) => { upsertCategoryThen(res); } )
                                        .catch( (err) => { upsertCatch(err); });        
            }
    ...
    onDeleteConfirm() {  
        ...
        dalCategories.deleteCategory(this.state.id).then( (response) => {
                        ...
                        <tr>
                            <td colSpan={2}>
                                <TextField  key="cbParentID" 
                                            ...
                                            select 
                                            label="ParentID" 
                                            value={ (this.state.category && this.state.category.ParentID) ? 
                                                        this.state.category.ParentID : '-1' }
                        ...
                        <tr>
                            <td colSpan={2}>
                                <FormControlLabel
                                    key="lblIsDeleted"                        
                                    control = {
                                        <Checkbox   checked={ this.state.category.IsDeleted ? true : false } 
                        ...
                        <tr>
                            <td colSpan={2}>
                                <TextField  key="cbCreatedByID" 
                                            ...
                                            select 
                                            label="CreatedByID" 
                                            value={ (this.state.category && this.state.category.CreatedByID) ? 
                                                        this.state.category.CreatedByID : '-1' }
                        ...
                        <tr>
                            <td colSpan={2}>
                                <TextField  key="cbModifiedByID" 
                                            ...
                                            select 
                                            label="ModifiedByID" 
                                            value={ (this.state.category && this.state.category.ModifiedByID) ? 
                                                        this.state.category.ModifiedByID : '-1' }
    ...
    async _getCategory()
    {
        if(this.state.id) {
            ...
            let dalCategories = new CategoriesDal();
            let response = await dalCategories.getCategory(this.state.id);

            if(response.status == constants.HTTP_OK)
            {
                updatedState.category = response.data;                
            }
